...
if (isset($_COOKIE['memory'])) {
    $igncookie = $_COOKIE['memory'];
    echo $igncookie;
    $autologin = $pdo->query("SELECT mdp from utilisateur where ign = '$igncookie'");
    $fetchautologin = $autologin->fetch(PDO::FETCH_OBJ);
    echo $fetchautologin->mdp;
