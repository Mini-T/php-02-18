            ...
            <div id="border">
                <div id="container">
                    ...
                    if (!empty($_POST['email'] and $_POST['Username'] and $_POST['mdp'])) {
                        ...
                        $hmac = hash_hmac('sha256', $cryptedPostmdp, $key, $as_binary = true);
                        $cryptedfinalmdp = base64_encode(openssl_random_pseudo_bytes($ivlen) . $hmac . $cryptedPostmdp);
                    ...
                    <?php 
                        $decryptionsql = $pdo->query("SELECT mdp From utilisateur where mdp = '$cryptedfinalmdp'");
                        $c = base64_decode($cryptedfinalmdp);
                        $ivlen = openssl_cipher_iv_length($cipher = "AES-128-CBC");
                        $iv = substr($c, 0, $ivlen);
                        $hmac = substr($c, $ivlen, $sha2len = 32);
                        $cryptedPostmdp = substr($c, $ivlen + $sha2len);
                        $decryptedmdp = openssl_decrypt($cryptedPostmdp, $cipher, $key, $options = OPENSSL_RAW_DATA, $iv);
