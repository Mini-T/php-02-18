...
<body>
    <form method="POST" action="#">
        ...
        if (!empty($_POST)) {
            ...
            $SelectCountI = $pdo->query("SELECT COUNT(*) FROM utilisateur WHERE email = '$postign'");
            $SelectCountP = $pdo->query("SELECT COUNT(*) FROM utilisateur WHERE email = '$postmdp'");
            
            $fetchassocshowM = $SelectCountM->fetch(PDO::FETCH_COLUMN);
            $fetchassocshowI = $SelectCountM->fetch(PDO::FETCH_COLUMN);
            $fetchassocshowP = $SelectCountM->fetch(PDO::FETCH_COLUMN);
            
            if ($fetchassocshowM > 0 and $fetchassocshowI > 0 and $fetchassocshowP > 0) {
                echo "Vous êtes déjà enregistré veuillez vous connecter";
            } else {
                try {
                    $pdo->exec($register);
                    echo 'enregistré' ;
